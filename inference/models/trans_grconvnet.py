...
class GenerativeResnet(GraspModel):
    ...
    def __init__(
        ...
        self.st3 = SpatialTransformer(
            channel_size * 4, 1, 1, 512
        )  # Add SpatialTransformer layer after res3
        self.res4 = ResidualBlock(channel_size * 4, channel_size * 4)
        self.st4 = SpatialTransformer(
            channel_size * 4, 1, 1, 512
        )  # Add SpatialTransformer layer after res4
        self.res5 = ResidualBlock(channel_size * 4, channel_size * 4)
        self.st5 = SpatialTransformer(
            channel_size * 4, 1, 1, 512
        )  # Add SpatialTransformer layer after res5
    ...
    def forward(self, x_in, cond):
        ...
        x = self.res3(x)
        x = self.st3(x, cond)  # Apply SpatialTransformer layer after res3
        x = self.res4(x)
        x = self.st4(x, cond)  # Apply SpatialTransformer layer after res4
        x = self.res5(x)
        x = self.st5(x, cond)  # Apply SpatialTransformer layer after res5
