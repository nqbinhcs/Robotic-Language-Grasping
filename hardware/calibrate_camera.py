...
class Calibration:
    def __init__(
        self, cam_id, calib_grid_step, checkerboard_offset_from_tool, workspace_limits
    ):
    ...
    def _get_rigid_transform_error(self, z_scale):
        ...
        observed_x = np.multiply(
            np.squeeze(self.observed_pix[:, [0]]) - self.camera.intrinsics.ppx,
            observed_z / self.camera.intrinsics.fx,
        )
        observed_y = np.multiply(
            np.squeeze(self.observed_pix[:, [1]]) - self.camera.intrinsics.ppy,
        ...
        R, t = self._get_rigid_transform(
            np.asarray(new_observed_pts), np.asarray(self.measured_pts)
        )
        t.shape = (3, 1)
        self.camera2world = np.concatenate(
            (np.concatenate((R, t), axis=1), np.array([[0, 0, 0, 1]])), axis=0
        )

        # Compute rigid transform error
        registered_pts = np.dot(R, np.transpose(new_observed_pts)) + np.tile(
            t, (1, new_observed_pts.shape[0])
        )
    ...
    def run(self):
        ...
        for tool_position in calib_grid_pts:
            ...
            if checkerboard_found:
                ...
                checkerboard_x = np.multiply(
                    checkerboard_pix[0] - self.camera.intrinsics.ppx,
                    checkerboard_z / self.camera.intrinsics.fx,
                )
                checkerboard_y = np.multiply(
                    checkerboard_pix[1] - self.camera.intrinsics.ppy,
        ...
        logging.info("Saving...")
        np.savetxt(
            "saved_data/camera_depth_scale.txt", camera_depth_offset, delimiter=" "
        )
